...
create view checkin_ticket_event_and_category_info as (
      ...
      where
            t.status in ('ACQUIRED', 'CHECKED_IN')
            and t.first_name is not null and (t.first_name <> '') IS TRUE
            and t.last_name is not null and (t.last_name <> '') IS TRUE
            and t.email_address is not null and (t.email_address <> '') IS TRUE
